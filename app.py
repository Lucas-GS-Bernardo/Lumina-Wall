...
print("--- TESTE DE CAMINHO (UPLOADS) ---")
print(f"UPLOAD_FOLDER (onde os arquivos DEVEM aparecer): {UPLOAD_FOLDER}")
print(f"Pasta existe? {os.path.exists(UPLOAD_FOLDER)}")
print(f"Posso escrever nela? {os.access(UPLOAD_FOLDER, os.W_OK)}")
print("-----------------------------------")
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        print(f"[UPLOAD] Tentando salvar em: {filepath}")
        try:
            file.save(filepath)
        except Exception as e:
            conn.close()
            print(f"[UPLOAD] ERRO ao salvar o arquivo: {e}")
            registrar_log("❌ Erro ao salvar arquivo", "DANGER", detalhes=str(e))
            return f"Erro ao salvar o arquivo no disco: {e}", 500

        if os.path.exists(filepath):
            print(f"[UPLOAD] OK: arquivo confirmado em disco ({os.path.getsize(filepath)} bytes)")
        else:
            conn.close()
            print("[UPLOAD] ERRO: file.save() não lançou exceção, mas o arquivo NÃO está no disco!")
            registrar_log("❌ Arquivo não encontrado após save", "DANGER", detalhes=filepath)
            return "Erro: o arquivo não foi encontrado no disco após o salvamento.", 500
...
if __name__ == '__main__':
    ...
    # --- CONFIGURAÇÃO DE REDE (DOCKER / REVERSE PROXY) ---
    PORTA = int(os.environ.get("PORT", 5000))
    DOMINIO = os.environ.get("DOMINIO", "mural.eanig.net")
    ...
    print(f"🔗 ACESSO: http://{DOMINIO} ou IP_DO_SERVIDOR:{PORTA}")
    print("═"*60)
    print(f"📡 Container pronto e escutando na porta {PORTA}...\n")

    # O host '0.0.0.0' é OBRIGATÓRIO no Docker para aceitar conexões externas
    try:
        from waitress import serve
        serve(app, host='0.0.0.0', port=PORTA, threads=10)
    except Exception as e:
